...
public class CFTree {
	 ...
	 */
	public CFTree(int maxNodeEntries, double distThreshold, FeatureManager fm, boolean applyMergingRefinement) {
		//if(distFunction < D0_DIST || distFunction > D4_DIST)
		//	distFunction = D0_DIST;
		this.fm = fm;
		root = new CFNode(maxNodeEntries,distThreshold,fm,applyMergingRefinement,true);
		leafListStart = new CFNode(0,0,fm,applyMergingRefinement,true); // this is a dummy node that points to the fist leaf
	 ...
	 */
	public boolean insertEntry(DigitalObject x) {
		instanceIndex++;
		...
		//System.out.println("inserEntry method: " + x + " " + instanceIndex);
	 ...
	 * @return true if insertion was successful
	 */
	public boolean insertEntry(DigitalObject x, int index) {
	...
	private boolean rebuildIfAboveMemLimit() {
		if(hasReachedMemoryLimit(this,memLimit)) {
			...
			CFTree newTree = this.rebuildTree(root.getMaxNodeEntries(), newThreshold, fm, root.applyMergingRefinement(), false);
			copyTree(newTree);
	...
	private void splitRoot() {
		...
		if(newEntry1.getClusterEntry() != null)
			newRoot.addToEntryList(newEntry1);
		if(newEntry2.getClusterEntry() != null)
	 ...
	 */
	public double computeNewThreshold(CFNode leafListStart,FeatureManager fm, double currentThreshold) {
		double avgDist = 0;
		int n = 0;
	 ...
	 */
	public CFTree rebuildTree(int newMaxEntries, double newThreshold, FeatureManager fm, boolean applyMergingRefinement, boolean discardOldTree) {
		CFTree newTree = new CFTree(newMaxEntries, newThreshold, fm, applyMergingRefinement);
	 ...
	 */
	public ArrayList<ArrayList<DigitalObject>> getSubclusterMembers() {
		ArrayList<ArrayList<DigitalObject>> membersList = new ArrayList<ArrayList<DigitalObject>>();
		...
		while(l!=null) {
			if(!l.isDummy()) {
				...
				for(CFEntry e : l.getEntries())
					membersList.add(e.getClusterEntry());
			}
			l = l.getNextLeaf();
		}
	 ...
	 */
	public int mapToClosestSubcluster(DigitalObject x) {
